...
delid.forEach(ind => {
    ...
    //             else obj.fullName = arr[i].firstName;
    //         }
...
// ------------------------
function name(arr) {
    return arr.LastName == null && arr.firstName == null || arr.LastName == "" && arr.firstName == "";
...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr.yearsOfExperience < 1 && name(arr))
...
const classesAvg = (data) => {
    // write your code here
    let d = data.grades.length;
    let f = data.grade.classes.lingth;
    let no
};
